...
Definition cmp_sat (Po : polyType) 
  (I : array int63)
  (x : array bigQ) (X : array (array bigQ)) 
  (l : int63):=
  let Al := Po.1.[l] in
  ILex.lex_func2_rel_ (Uint63.succ (length Po.1))
  ...
  (fun k i=> 
    if (i =? 0)%uint63 then (Po.2.[l],true) else
    if (Uint63.pred i =? l)%uint63 then ((-1)%bigQ,true) else
    (0%bigQ,true)
  ).
  
Definition cmp_vect (Po : polyType) (I : array int63) (x : array bigQ) (X : array (array bigQ)) :=
  let cmp := PArrayUtils.mk_fun 
    (fun l=> cmp_sat Po I x X l)
    (length Po.1) Eq
  in 
...
Definition n := length Po.1.[0%uint63].
